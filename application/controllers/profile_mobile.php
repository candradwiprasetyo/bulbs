...
class Profile_mobile extends CI_Controller {
	...
	public function index() {
			...
			$result = $this->profile_mobile_model->read_id($this->session->userdata('user_id'));
			
			if($result){
				$data_creatives  = $result;
			}
			
			$data_creatives['follower'] = $this->profile_mobile_model->get_profile_follower($this->session->userdata('user_id'));
			$data_creatives['following'] = $this->profile_mobile_model->get_profile_following($this->session->userdata('user_id'));
			$data_creatives['view'] = $this->profile_mobile_model->get_profile_view($this->session->userdata('user_id'));
			$data_creatives['like'] = $this->profile_mobile_model->get_profile_like($this->session->userdata('user_id'));
			
			$this->load->view('layout/header', array('list' => $list, 'data' => $data));
			$this->load->view('profile_mobile/content', array('data_creatives' => $data_creatives));
	...
	public function edit() {
			...
			$this->load->view('layout/header', array('list' => $list, 'data' => $data));
			$this->load->view('profile_mobile/edit_form', array('data_creatives' => $data_creatives));
			$this->load->view('layout/footer'); 
		}else{
			redirect('login');
		}
	...
	public function save_profile() {
		...
		if($this->input->post('i_current_password') && $this->input->post('i_new_password')){
			$get_current_password = $this->profile_mobile_model->get_current_password($id);
		...
		$this->profile_mobile_model->save_profile($data, $id);
		$this->profile_mobile_model->save_account($data_account, $id);
		
		
		// hapus profile detail categories
		$this->profile_mobile_model->delete_detail($id);
		...
		while($r_p_category = mysql_fetch_array($q_p_category)){
			...
			if($this->input->post('i_pc_'.$r_p_category['pc_id'])){
				...
				$this->profile_mobile_model->save_detail($data_detail);
			}
